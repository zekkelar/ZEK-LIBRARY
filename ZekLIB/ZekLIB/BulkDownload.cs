...
namespace ZekLIB
{
    public partial class BulkDownload : Form
    {
        ...
        private void setting_up()
        {
            ...
            description_download1.Visible = false;
            description_download2.Visible = false;
            description_download3.Visible = false;
            remove_download1.Visible = false;
            remove_download2.Visible = false;
            remove_download3.Visible = false;
            view_download1.Visible = false;
            view_download2.Visible = false;
            view_download3.Visible = false;
        ...
        private async Task get_all_download_internal()
        {
            richboxTitle = new RichTextBox[] { richTextBox1, richTextBox2, richTextBox3 };
            richboxDescription = new RichTextBox[] { description_download1, description_download2, description_download3 };
            item_picture = new PictureBox[] { pictureBox1, pictureBox2, pictureBox3 };
            view_button = new GunaButton[] { view_download1, view_download2, view_download3 };
            remove_button = new GunaButton[] { remove_download1, remove_download2, remove_download3 };
        ...
        private async Task get_all_download_external()
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                else
                {
                    ...
                    foreach (var book in _getbooks.data)
                    {
                        ...
                        link_download.Add(book[5]);
                        description.Add(book[2]);
                        added_date.Add(book[0]);
                        title_.Add(book[8]);
                        language__.Add("English");
                        author_list.Add(book[1]);
                        source.Add(book[7]);
                        img_urls.Add(book[3]);

                        richboxTitle[cont].Text = book[8];
                        richboxTitle[cont].Visible = true;

                        richboxDescription[cont].Text = book[2];
                        richboxDescription[cont].Visible = true;

                        item_picture[cont].ImageLocation = book[3];
                        item_picture[cont].Visible = true;
                        ...
                        view_button[cont].Click += Button_Click;
                        view_button[cont].Tag = book[7] + "|" + "view_product";

                        remove_button[cont].Click += Button_Click;
                        remove_button[cont].Tag = book[8] + "|" + "remove_product";
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            ...
            if (source_now == "internal source")
            {
                ...
                {
                    _ = remove_download(data_parser[0]);
                }
            }
            else
            {
        ...
        private async Task remove_download(string name_img)
        {
            string url = $"{dataInstance.Myurl}download";
            if (source_now == "internal source")
            {
                        ...
                        while (downloadQueue.Count > 0)
                        {
                            if (tasks.Count < 5)
                            {
                                ...
                                if (i != -1)
                                {
                                    string downloadLink = downloadQueue.Dequeue();
                                    progressBars[i].Value = 1;
                                    var task = DownloadFileAsync(httpClient, progressBars[i],
                                        downloadLink, _getbooks.data[i][8], 
                                        progressLabels[i], speedLabels[i], 
                                        downloadedLabels[i], statusLabel[i], 
                                        NamaFileLabel[i]);
                                    tasks.Add(task);
                                }
                            }
                                  ...
                                  Label labelSpeed, Label downloadedLabel, Label statusLabel, Label NamaFile)
        {
            ...
            using (var stream = await response.Content.ReadAsStreamAsync())
            {
                ...
                using (var fileStream = new FileStream($"{Application.StartupPath}\\BulkDownloads\\{fileName}", FileMode.Create, FileAccess.Write, FileShare.None))
                {
        ...
        private void display_item_external()
        {
            ...
            foreach (var book in currentPageItems)
            {
                ...
                remove_button[cont].Click += Button_Click;
                remove_button[cont].Tag = book[7] + "|" + "remove_product";
        ...
        private void RemoveAllDownload_Click(object sender, EventArgs e)
        {
        ...
        private async Task remove_all_download()
        {
            ...
            if (source_now == "internal source")
            {
                ...
                {
                    {"username", loginInformation.Username},
                    {"id", "remove_all_download_internal"}
                };
            ...
            else
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        {"username", loginInformation.Username},
                        {"id", "remove_all_download_external"}
                    };
        ...
        private void remove_download1_Click(object sender, EventArgs e)
        {
